...
class Roles extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, alias, level, i_inserted, i_updated', 'required'),
			array('level, parts, i_flag_deleted, i_disable, i_inserted, i_updated', 'numerical', 'integerOnly'=>true),
			array('title, alias', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, title, alias, level, parts, i_flag_deleted, i_disable, i_inserted, i_updated', 'safe', 'on'=>'search'),
		);
	}
